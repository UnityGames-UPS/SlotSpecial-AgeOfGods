...
public class SlotController : MonoBehaviour
{
  ...
  private List<WildIconView> activeWildFeatureIcons = new List<WildIconView>();

  void Start()
  {
    for (int col = 0; col < WildMatrix.Count; col++)
      for (int row = 0; row < WildMatrix[col].slotImages.Count; row++)
        WildMatrix[col].slotImages[row].Init(slotMatrix[col].slotImages[row]);
  }
      ...
      for (int i = 0; i < goldPositions.Count; i++)
      {
        ...
        for (int j = 0; j < goldPositions[i].positions.Count; j++)
        {
          ...
          if (id == 10)
          {
            ...
            WildMatrix[col].slotImages[row].SetGoldIcon(goldIconImages[id]); // handles overlay visual
          }
          else
          {
  ...
  internal IEnumerator HideGoldIcons()
  {
    ...
    for (int i = 0; i < WildMatrix.Count; i++)
    {
      ...
      {
        if (WildMatrix[i].slotImages[j].isGold)
          WildMatrix[i].slotImages[j].AnimateGoldIcon();
      }
    }
  ...
  internal void ResetWildMatrix()
  {
    foreach (var col in WildMatrix)
      foreach (var wildIcon in col.slotImages)
        wildIcon.Reset();
  }
  ...
  internal IEnumerator PlayWildFeatureAnimations(List<string> wildPositions)
  {
    if (wildPositions == null || wildPositions.Count == 0) yield break;

    ToggleDarkFG(true, false);

    var ordered = new List<Vector2Int>();
    foreach (var s in wildPositions)
    {
      if (string.IsNullOrEmpty(s))
      {
        Debug.LogError($"Wild position is empty: '{s}'");
        continue;
      }
      var parts = s.Split(',');
      if (parts.Length < 2 || !int.TryParse(parts[0].Trim(), out int row) || !int.TryParse(parts[1].Trim(), out int col))
      {
        Debug.LogError($"Wild position malformed: '{s}'");
        continue;
      }
      if (col < 0 || col >= WildMatrix.Count || row < 0 || row >= WildMatrix[col].slotImages.Count)
      {
        Debug.LogError($"Wild position out of range (row={row}, col={col}): '{s}'");
        continue;
      }
      if (WildMatrix[col].slotImages[row].isGold)
        continue; // already shown as gold-wild overlay; don't double-animate
      ordered.Add(new Vector2Int(row, col));
    }
    ...
    {
      var pos = ordered[i];
      var wildIcon = WildMatrix[pos.y].slotImages[pos.x];
      activeWildFeatureIcons.Add(wildIcon);
      running.Add(StartCoroutine(wildIcon.PlayWildFeatureAnimation()));
      if (i < ordered.Count - 1)
        yield return new WaitForSecondsRealtime(wildPerColumnDelay);
    }
  ...
  internal void ResetWildFeatureIcons()
  {
    foreach (var wildIcon in activeWildFeatureIcons)
      if (wildIcon != null) wildIcon.ResetWildFeatureAnimation();
    activeWildFeatureIcons.Clear();
  }
...
{
  public List<WildIconView> slotImages = new List<WildIconView>(10);
}
