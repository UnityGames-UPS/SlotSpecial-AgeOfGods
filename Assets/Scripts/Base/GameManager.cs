...
public class GameManager : MonoBehaviour
{
  ...
  [SerializeField] private AutoSpinPanelController autoSpinPanel;
  ...
  void Start()
  {
    ...
    {
      if (autoSpinPanel != null) autoSpinPanel.CloseIfOpen();
      if (autoSkipArmed)
      {
    ...
    SetButton(ToatlBetMinus_Button, () => { if (autoSpinPanel != null) autoSpinPanel.CloseIfOpen(); OnBetChange(false); });
    SetButton(TotalBetPlus_Button, () => { if (autoSpinPanel != null) autoSpinPanel.CloseIfOpen(); OnBetChange(true); });
    SetButton(TurboON_Button, () => { if (autoSpinPanel != null) autoSpinPanel.CloseIfOpen(); ToggleTurboMode(); });
    SetButton(TurboOFF_Button, () => { if (autoSpinPanel != null) autoSpinPanel.CloseIfOpen(); ToggleTurboMode(); });
    ...
    uIManager.CloseAutoSpinPanel = () => { if (autoSpinPanel != null) autoSpinPanel.CloseIfOpen(); };
    uIManager.OnLowBalConfirm = () => ToggleButtonGrp(true);
    socketController.ShowDisconnectionPopup = uIManager.DisconnectionPopup;
    socketController.OnBalanceSynced = UpdateBalanceDisplay;
  }
  ...
  void InitGame()
  {
    ...
    wheelController.PopulateWheelValues(socketController.InitData.features.bonus);
  }
  ...
  internal void StartAutoSpin(int count)
  {
    ...
    if (autoSpinPanel != null) autoSpinPanel.ShowStopButton(count);
  ...
  void HandleAutoUntilFeatureCutoff()
  {
    ...
    if (autoSpinPanel != null) autoSpinPanel.HideStopButton();
  }
  ...
  IEnumerator AutoSpinRoutine()
  {
    ...
    while (isAutoSpin)
    {
      if (autoSpinPanel != null) autoSpinPanel.UpdateStopCount(_autoSpinRemaining);
    ...
    while (true)
    {
      if (!isFreeSpin && LastSpinWasFreeSpinTrigger())
      {
        ...
        yield return freeSpinController.RunFreeSpins(
          ...
          () => socketController.ResultData?.payload?.freeSpinsRemaining ?? 0,
          LastSpinWasWildTrigger,
  ...
  IEnumerator OnSpin()
  {
    ...
    if (hasWild)
    {
      ...
      yield return slotManager.PlayWildFeatureAnimations(wildPositions);
  ...
  // Snapped (not tweened) — an external balance push is a correction, not a spin-result win.
  ...
  {
    if (isSpinning || isFreeSpin) return;
    ...
    else if (!uIManager.IsLowBalPopupOpen && !isAutoSpin)
    {
      ToggleButtonGrp(true);
    }
  }
