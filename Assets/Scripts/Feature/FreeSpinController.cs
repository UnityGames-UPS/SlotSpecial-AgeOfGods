...
public class FreeSpinController : MonoBehaviour
{
  ...
  internal IEnumerator RunFreeSpins(int totalCount, Func<IEnumerator> spinFn, Func<double> getLastWinAmount,
    Func<bool> wasFreeSpinTrigger, Func<int> getRetriggerAward,
    Func<int> getServerSpinsRemaining, Func<bool> wasWildTrigger,
    Action<double> onOkBeforeFade, Action onAllDone)
  {
    ...
    while (_spinsRemaining > 0)
    {
      ...
      bool wildThisSpin = wasWildTrigger != null && wasWildTrigger();
      if (wildThisSpin) _spinsRemaining++;

      if (getServerSpinsRemaining != null)
      {
        int serverRemaining = getServerSpinsRemaining();
        if (serverRemaining != _spinsRemaining)
        {
          Debug.LogWarning($"[FreeSpin] count desync: local={_spinsRemaining}, server={serverRemaining}, wild={wildThisSpin}. Trusting server.");
          _spinsRemaining = serverRemaining;
        }
      }
      ...
      if (wasFreeSpinTrigger != null && wasFreeSpinTrigger())
      {
        ...
        if (getServerSpinsRemaining != null)
        {
          ...
          if (serverRemaining != _spinsRemaining)
          {
            Debug.LogWarning($"[FreeSpin] count desync after retrigger: local={_spinsRemaining}, server={serverRemaining}. Trusting server.");
            _spinsRemaining = serverRemaining;
          }
        }
      }
