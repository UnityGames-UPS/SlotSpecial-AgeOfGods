mergeInto(LibraryManager.library, {
    ...
    SendPostMessage: function(messagePtr) {
      ...
      console.log('[JS] SendPostMessage: ' + message);
      ...
      function sendDimensionsToUnity() {
        try {
          ...
          if (typeof SendMessage === 'function') {
            ...
            console.warn('[JS] Unity SendMessage not available for resize');
          }
        } catch (err) {
    ...
    RegisterTokenListener: function(gameObjectNamePtr, methodNamePtr) {
      var gameObjectName = UTF8ToString(gameObjectNamePtr);
      var methodName = UTF8ToString(methodNamePtr);
      console.log('[JS] RegisterTokenListener called for:', gameObjectName + '.' + methodName);
